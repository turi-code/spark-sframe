...
class GraphLabUtilTestSuite extends FunSuite with BeforeAndAfter {
  ...
  test("save a dataframe of StructType to an sframe") {
    ...
    val outputFname = GraphLabUtil.toSFrame(df, tmpDir.toString, "test")
    val rdd = GraphLabUtil.toRDD(sc, outputFname).cache
    println(rdd.collect()(0).toString)
  }
   ...
   val schema  = StructType(Seq(StructField("name", StringType,false),
    ...
    case class Rate(rate: Long, comment: String)
    case class Person(name: String, age: Long, rate: Row)
    var temprdd  = sc.parallelize(Seq(Person("Andy", 32, Row(Rate(1,"haha")))))
    val row_rdd = temprdd.map(a => Row(a))
    val df = sqlContext.createDataFrame(row_rdd,schema)
    val tmpDir = Files.createTempDirectory("sframe_test")
    val outputFname = GraphLabUtil.toSFrame(df, tmpDir.toString, "test")
    val rdd = GraphLabUtil.toRDD(sc, outputFname).cache
